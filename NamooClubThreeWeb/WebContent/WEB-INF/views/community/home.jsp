				...
				<ul class="list-group">					
					<c:forEach var="community" items="${belongCommunities}" varStatus="list">
						...
						${community.description }<br/>
							...
							<button type="button" class="btn btn-default btn-sm" onclick = "location.href='clList.xhtml'">상세</button>
						</li>
					</c:forEach>
				</ul>
				</div>
				 ...
				 <div class="tab-pane fade" id="unjoined">	
					...
					<c:forEach var="community" items="${communities}" varStatus="list">
						<li class="list-group-item">${community.name}<br />
						${community.description }<br/>
							<button type="button" class="btn btn-default btn-sm" onclick = "view/community/join.xhtml/?cmName=${community.name}">커뮤니티 가입</button>	
						</li>
					</c:forEach>
				</ul>
				</div>
